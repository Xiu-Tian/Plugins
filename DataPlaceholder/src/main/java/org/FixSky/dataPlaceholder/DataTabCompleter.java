...
public class DataTabCompleter implements TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            List<String> subCommands = Arrays.asList("help", "setdata", "adddata", "removedata", "getdata", "test");
            suggestions = subCommands.stream()
                    .filter(s -> s.startsWith(args[0].toLowerCase()))
                    .collect(Collectors.toList());
        } else if (args.length >= 2) {
            String sub = args[0].toLowerCase();
            if (sub.equals("setdata") || sub.equals("adddata") || sub.equals("removedata") || sub.equals("getdata") || sub.equals("test")) {
