...
public class DataCommand implements CommandExecutor {
    ...
    private void sendHelp(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.YELLOW + "/dp help");
        sender.sendMessage(ChatColor.YELLOW + "/dp setdata <键> <值>");
        sender.sendMessage(ChatColor.YELLOW + "/dp setdata <玩家> <键> <值>");
        sender.sendMessage(ChatColor.YELLOW + "/dp adddata <键> <数值>   (仅支持数字累加)");
        sender.sendMessage(ChatColor.YELLOW + "/dp adddata <玩家> <键> <数值>");
        sender.sendMessage(ChatColor.YELLOW + "/dp removedata <键>");
        sender.sendMessage(ChatColor.YELLOW + "/dp removedata <玩家> <键>");
        sender.sendMessage(ChatColor.YELLOW + "/dp getdata <键>");
        sender.sendMessage(ChatColor.YELLOW + "/dp getdata <玩家> <键>");
    ...
    // ---------- adddata (仅支持数值累加) ----------
    private void handleAddData(CommandSender sender, String[] args) {
        if (args.length < 3) {
            sender.sendMessage(ChatColor.RED + "用法: /dp adddata <键> <数值> 或 /dp adddata <玩家> <键> <数值>");
            return;
        }
        ...
        if (possiblePlayer != null && args.length == 4) {
            ...
            key = args[2];
            try {
                addValue = Double.parseDouble(args[3]);
            } catch (NumberFormatException e) {
                sender.sendMessage(ChatColor.RED + "参数 <数值> 必须是一个数字。");
                return;
            }
        } else if (args.length == 3) {
            if (!(sender instanceof Player)) {
                sender.sendMessage(ChatColor.RED + "控制台必须指定玩家名: /dp adddata <玩家> <键> <数值>");
                return;
            }
            target = (Player) sender;
            key = args[1];
            try {
                addValue = Double.parseDouble(args[2]);
            } catch (NumberFormatException e) {
                sender.sendMessage(ChatColor.RED + "参数 <数值> 必须是一个数字。");
                return;
            }
        ...
        String oldValue = plugin.getDataManager().getData(target, key);
        double oldNum;
        if (oldValue == null) {
            oldNum = 0.0;  // 不存在则视为0
        } else {
            try {
                oldNum = Double.parseDouble(oldValue);
            } catch (NumberFormatException e) {
                sender.sendMessage(ChatColor.RED + "玩家 " + target.getName() + " 的 " + key + " 当前值不是数字，无法累加。");
                return;
            }
        }

        double newNum = oldNum + addValue;
        // 如果结果是整数则显示为整数，否则显示小数
        String finalValue = (newNum == (int) newNum) ? String.valueOf((int) newNum) : String.valueOf(newNum);

        plugin.getDataManager().setData(target, key, finalValue);
        sender.sendMessage(ChatColor.GREEN + "已为 " + target.getName() + " 累加 " + key + "，新值 = " + finalValue);
    }
